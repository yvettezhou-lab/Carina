...
export function Transactions() {
  ...
  async function refresh() {
    const [tx, transfers, accountRows, categoryRows, personRows] = await Promise.all([
      ...
      db.transfers.orderBy('dateTime').reverse().toArray(),
      db.accounts.toArray(),
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter(item => {
      ...
      if (flowFilter !== 'all' && flowFilter !== 'transfer' &&
      ...
      if (
        ...
        !(item.otherAccount ?? '').toLowerCase().includes(q)
      ) return false;
      return true;
    });
  ...
  return (
    <section>
      ...
      <div className="ledger-filter-toolbar">
        ...
        {activeFilterCount > 0 && (
          <button type="button" className="ledger-clear-filter" onClick={clearFilters}>Clear</button>
        )}
      ...
      {showFilters && (
        <div className="ledger-filter-panel">
          <div className="ledger-filter-head">
            <div>
              <span className="panel-kicker">LEDGER FILTER</span>
          ...
          <div className="ledger-filter-grid">
            <label className="ledger-filter-field">
              <span>Type</span>
              <select value={flowFilter} onChange={e => setFlowFilter(e.target.value as typeof flowFilter)}>
              ...
              <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)}>
                ...
                {categories.map(x => (
                  <option key={x.id} value={x.id}>
                    {x.name} · {x.flow === 'income' ? 'Income' : 'Expense'}
                  </option>
                ))}
              </select>
            </label>
          ...
          <div className="ledger-filter-actions">
            ...
            <button type="button" className="active" onClick={() => setShowFilters(false)}>Done</button>
          </div>
        </div>
      )}
      ...
      <div className="ledger-list">
        {filteredItems.length === 0 ? (
          <div className="empty">
            <span className="empty-script">The ledger is quiet</span>
              ...
              <div className="ledger-copy">
                ...
                <span>
                  {item.kind === 'transfer'
                    ...
                    : `${item.category} · ${item.account ?? 'Unknown'}`}
                </span>
              </div>

              <div className="ledger-right">
                <b className={item.kind === 'transfer' ? 'transfer-amount' : item.flow}>
                  {item.kind === 'transfer' ? '⇄ ' : item.flow === 'expense' ? '− ' : '+ '}¥{item.amount.toFixed(2)}
                </b>
